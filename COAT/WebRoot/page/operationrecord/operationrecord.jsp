...
function select(pagenow){
				...
				$.ajax({
				type: "post",
				url:basepath+"/operationrecord/OperationRecordServlet",
				data: {"method":"select",'username':$("#username").val(),'modular':$("#modular").val(),
					'startDate':$("#start_date").val(),'endDate':$("#end_date").val(),
					'operation':$("#operation").val(),'status':$("#status").val(),
					'pagenow':pagenow},
				beforeSend: function(){
						parent.showLoad();
				},
				...
				success:function(data){
					...
					if(dataRole[3]>0){
						...
						for(var i=0;i<dataRole[0].length;i++){
							...
							html+="<tr id='select' title='"+i+"'><td >"+((i+1)+(dataRole[2]-1)*dataRole[4])+"</td><td align='center'>"+dataRole[0][i].username+"</td><td >"
							+dataRole[0][i].ipaddress +"</td><td style='text-align:left;'>"+(dataRole[0][i].httpaddress.replace('<%=basePath%>',''))+ "</td><td >"+dataRole[0][i].modular +"</td><td >"
							+dataRole[0][i].operation +"</td><td >"+dataRole[0][i].date +"</td><td >"+dataRole[0][i].result+"</td><td title='"+dataRole[0][i].remark1+"'>"
							+(("<a href='javascript:void(0);' onclick='detail("+i+","+((i+1)+(dataRole[2]-1)*dataRole[4])+")'  style='text-decoration: none;' >详情</a>") )
							+"</td></tr>";		 
						}
			                $(".page_and_btn").show();
...
/*****************************表单结束********************************/	
  ...
  function detail(index,id)
  {
	...
	$.dialog( {
		...
		min : false,
		resize : false,
		content : "url: "+'page/operationrecord/operationrecord_detail.jsp'
	});
  }
 </script>
	...
	<div class="info-search">
		<table>
			...
			<tr>
				...
				<td class="tagName">Modular：</td>
				<td class="tagCont">
					<input type="text" name="modular" id="modular">
				</td>
			</tr>
			<tr>
	...
	<div class="info-table">
		<table id="jqajax">
			...
			<tr id="title">
				...
				<th class="width_10">Ip Address</th>
				<th class="width_10">Http Address</th>
				<th class="width_5">Modular</th>
				<th class="width_5">Operation</th>
				<th class="width_10">Date</th>
				<th class="width_5">Result</th>
				<th class="width_20">Operation</th>
			</tr>
			</thead>
		</table>
