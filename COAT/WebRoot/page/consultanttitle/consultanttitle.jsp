...
<html>
  <head>
    ...
    <title>My JSP 'consultanttile.jsp' starting page</title>
  ...
  <body>
    <div class="cont-info">
    	<div class="info-search">
    		<table>
    			<tr>
    				<td class="tagName">英文职位名称：</td>
    				<td class="tagCont">
    					<input type="text" name="position_ename" id="position_ename" />
    				</td>
    				<td class="tagName">中文职位名称：</td>
    				<td class="tagCont">
    					<input type="text" name="position_cname" id="position_cname" />
    				</td>
    			</tr>
    	...
    	<div class="info-table">
    		<table>
    			...
    			<tr>
    				...
    				<th class="width_25">英文职位名称</th>
    				<th class="width_15">中文职位名称</th>
    				<th class="width_15">添加时间</th>
    				<th class="width_15">添加人</th>
    ...
    <script type="text/javascript">
    	...
    	$(function(){
			...
			$('#add').click(function(){
				addlhg(400, 280, basepath+"page/consultanttitle/consultanttitle_info.jsp" );
			});
    	});
    	...
    	function select(pageNow){
   			...
   			$.myAjax("QueryConsultantTitleServlet", data, function(result){
   				var html = ""; 
   				//result = eval("("+result+")"),
   				if(parseInt(result.data.total) > 0){	
   					...
   					for(var i=0; i<lists.length; i++){
	   					html += '<tr>'+
   									'<td>'+(i+1)+'</td>'+
   									'<td>'+lists[i].position_ename+'</td>'+
   									'<td>'+lists[i].position_cname+'</td>'+
   		...
   		function modify(i){
   			...
   			editlhg(400, 280, basepath+"page/consultanttitle/consultanttitle_modify.jsp", data);
   		}
		...
		function exportDate(){
			if(datalists != null){
				...
				location.href= "QueryConsultantTitleServlet?&position_ename="+ename+"&position_cname="+cname+"&position_sfyx="+sfyx+"&method=export";
			}else{
				alert("请先查询数据再进行导出操作");
			}
		}
		function del(index){
			if(datalists[index].sfyx=="N"){
				error("该数据已为失效状态,删除失败.");
				return false;
			}
			var id = datalists[index].Id;
			$.dialog.confirm("是否删除该条数据？",function(){
				var data = {'id':id,method:'delete'};
				myAjaxs("QueryConsultantTitleServlet",data);
			});
		}
    </script>
  </body>
