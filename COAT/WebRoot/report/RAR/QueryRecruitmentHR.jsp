...
-->
              ...
              if(str>$("#date").val()){
                ...
                if(confirm('Are you sure Comfirmation Request ?')){
                     $.ajax({
                       url:"QueryRecruitmentHRServlet", 
                       type:"post",
                       data:{"refno":$("#refno").val(),"date":$("#date").val(),"method":"save"},
                       success:function(date){
                           if(date!="error"){
                              alert("success");
                              ...
                              var date1=reduceByTransDate(date)                    
                              var date2=date;
                              var Person=temp[0].contactperson;
                              var email=temp[0].contactemail;
                              var mediaName=temp[1].medianame;
                              email_confirmation(staffname,date1,date2,Person,email,mediaName);
                             //刷新页面
                              hideview();
                        }
                       else{
 ...
 function getPayment(){
    ...
    //非空验证
     if($("#payDate").val()==""){     
         ...
         if(str>$("#payDate").val()){
          ...
          alert("Please write Handled");
           return;
       }if($("#saleno").val()==""){
          alert("Please write saleno");
           return ;
       } 
       if(confirm('Are you sure completed this order')){    
        $.ajax({
          url:"QueryRecruitmentHRServlet",
          type:"post",
          data:{"refno":$("#refno").val(),"staffname":$("#name").val(),"saleno":$("#saleno").val(),"paymentMethod":$("#paySelectId").val(),"paymentAount":$("#moneyId").val(),"payDate":$("#payDate").val(),"handled":$("#handledId").val(),"method":"savePayment"},
          success:function(date){
             if(date=="success"){
                 alert("success");
                  var staffname=temp[0].staffname;        
                  ...
                  var staffcode=temp[0].staffcode;
           eamil_conpleted(money,staffname,staffcode,mediaName,date2,Person,email);   
                hideview();
           }
            else{
                alert("failed");
   ...
   //下一个日期
   function reduceByTransDate(dateParameter) {
    ...
    function   export1(){
      ...
      var method="export1";
      location.href="QueryRecruitmentHRServlet?date1="+date1+"&date2="+date2+"&staffcode="+staffcode+"&refno="+refno+"&staffname="+staffname+"&status="+status+"&method="+method;
    }
  </script>
  ...
  <div align="center" style="color:#00305A;font-family: 'Arial Narrow';font-size:16px; top:0px;bottom: 20px;absolute;width: 100%;"><strong>
	    ...
	    <div style="width: 100%;"><p align="center">  
	    	  <input type="button" name ="bbb" id="Confirm" value="Confirmation Request" onClick="Ready()"/> &nbsp;&nbsp;&nbsp;&nbsp;
	    	   <input type="button" name ="zzz" id="Scheduled" value="Scheduled" onClick="Demo('Scheduled')"/> &nbsp;&nbsp;&nbsp;&nbsp;
	    	    <input type="button" name ="aaa" id="Completed" value="Completed" onClick="getPayment();"/> &nbsp;&nbsp;&nbsp;&nbsp;
	    	  <input type="button" name ="ddd" id="void" value="VOID" onClick="Demo('VOID')"/> &nbsp;&nbsp;&nbsp;&nbsp;
  ...
  <table width="100%"  border="0" cellpadding="0" cellspacing="0" class="table-ss" background="css/officeAdmin_bg.gif">
            ...
            <tr>
              ...
              <td  align="right"><input id="searchs" name="search" type="submit" value="Search" align="right"/>                     
               ...
               <input id="exp" name="search" type="submit" value="export" align="right" onclick="export1()"/></td>
            </tr>
    </table>
