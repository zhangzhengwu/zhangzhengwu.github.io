...
<html>
  ...
  function selects_last(pageNow){
			...
			data: {'method':'queryProduct','start_Date':$("#start_date").val(),'end_Date':$("#end_date").val(),
					'productcode':$("#productcode").val(),'BLBZ':$("#ordertype").val(),"productname":$("#productname").val(),'curretPage':pageNow},
			beforeSend: function(){
						parent.showLoad();
			},
			complete: function(){
			...
			success:function(data){
				...
				if(dataRole[3]>0){
					...
					for(var i=0;i<dataRole[0].length;i++){
						if(dataRole[0][i].BLBZ=="A"){
							BLBZ="有效"
						}else{
							BLBZ="无效"
						}
						html+="<tr id='select' title='"+i+"'><td align='center'>"+dataRole[0][i].procode+"</td><td  align='left'>"+
						dataRole[0][i].englishname+"</td><td>"+dataRole[0][i].unitprice+"</td><td align='left'>"+dataRole[0][i].c_clubprice+"</td>"
						+"<td align='center'>"+dataRole[0][i].specialprice+"</td><td align='center'>"+dataRole[0][i].quantity+"</td>" +
						"</td><td align='center'>"+dataRole[0][i].unit+"</td>" +"<td align='center'>"+dataRole[0][i].updates
						+"</td><td align='center'>"+BLBZ+"</td></tr>";
					}
				}
				else{
					$(".page_and_btn").hide();
	...
	<div class="info-table">
		<table id="jqajax">
			...
			<tr>
				...
				<th class="width_10">Unit Price</th>
				<th class="width_10">Club Price</th>
				<th class="width_10">Special Price</th>
				<th class="width_10">Quantity</th>
				<th class="width_10">Unit</th>
				<th class="width_15">UpDates</th>
				<th class="width_10">State</th>
			</tr>
			</thead>
		</table>
		<div align="center" class="page_and_btn">			<table class="table-ss" width="100%" border="0" cellpadding="0"
				cellspacing="0">
				<tr class="main_head">
