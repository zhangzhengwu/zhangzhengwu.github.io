...
<html>
  ...
  <base href="<%=basePath %>"></base>
    ...
    <title>Consultant New Marketing Premium Form</title>
	...
	function overas(types,i,n){
		...
		function resize(){
			//$("#order,#divId,.span_menu,#topTable").width($("body").width()-25);
		}
		...
		$(function(){
			...
			**/
			...
			$("#save").click(function(){
				...
				/**if($("#clubid").val() == "" && $("#chequeid").val() == "" && $("#cashid").val() == ""){
				...
				//	alert(sale_record+"=====totalMoney:=="+totalMoney);
				if(sale_record==""){
					return false;
				}  
			$.ajax({
				...
				"c-club":$("#c-club").val(),'location':$("#locationid").val(),
				'payMethod':$("#payMethod").val(),"chequeNo":$("#chequeNo").val(),
				"Banker":$("#Banker").val(),"totalMenoy":totalMoney,"sale_record":sale_record,"cah_number":$("#cashid").val(),"club_nubmer":$("#clubid").val(),"cheque_number":$("#chequeid").val(),"ordertype":"Consultant"},
				success:function(date){
					if(date=="success"){//订单保存成功
						...
						alert("Abnormal orders submitted1!");
						return false;
					}else if(date=="order"){//订单表保存失败
						alert("Abnormal orders submitted2!");
						return false;
					}else if(date=="record"){//记录表保存失败
						alert("Abnormal orders submitted3!");
						return false;
					}else{//超出库存producode
						stock=eval(date);
						reValue("unit");
			...
			<tr>
				...
				<td class="tagCont">
					<label class="inline checkbox">
						<input id="c-club" type="checkbox" value="N"/>YES
					</label>
				</td>
			</tr>
		</table>
