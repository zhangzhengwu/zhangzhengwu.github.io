			...
			success:function(data){
				...
				if(dataRole[0].length>0){
					...
					for(var i=0;i<dataRole[0].length;i++){
						...
						dataRole[0][i].location+"</td><td align='center' style='display:none;'>HK$ "+
						...
						/**if(comptime(dataRole[0][i].orderdate,GetDateStr("${SystemAcceTime}".substring(0,10).split("-"),-1)+" 12:00:00")==1 && parseFloat(("${SystemAcceTime}").substring(11,13))<12){
							html+="&nbsp;<a href='javascript:void(0);' onclick='del("+i+");' name ='2'>Delete</a>";
						}else if(comptime(dataRole[0][i].orderdate,"${SystemAcceTime}".substring(0,10)+" 12:00:00")==1 && parseFloat(("${SystemAcceTime}").substring(11,13))>=12){
							html+="&nbsp;<a href='javascript:void(0);' onclick='del("+i+");' name ='2'>Delete</a>";
						}**/
					...
					//html+="<tr  id='select'><td colspan='7' style='color: red;'><span>Remark:&nbsp;&nbsp;1.Daily Cut-Off time : 12:00noon." +
			 	...
			 	data:{'ordercode':downs[0][i].ordercode,'type':"Stationery"},
		  	 	success:function(date){
					...
					for(var i=0; i< d[0].length; i++){
						var num = i+1;
						html_left+="<tr id='left"+i+"' style='line-height:20px;' class='product'><td >"+
							num+"</td><td align='left'><input type='hidden' id='ordId' value='"+d[0][i].ordercode+"'/>"+
							d[0][i].ordercode+"</td><td  align='left'>"+
							d[0][i].procode+"</td><td  align='left'>"+
							d[0][i].proname+"</td><td  style='display:none;'>HK$ "+
							d[0][i].price+"</td><td>1</td><td>"+
							d[0][i].quantity+"</td><td  style='display:none;'>HK$ "+
							d[0][i].priceall+"</td>"+
						"</tr>";
					}
	...
	<div class="info-form">
		...
		<table>
			...
			<tr>
				<td class="tagName">Status：</td>
				<td class="tagCont">
					<input type="text" name="dstatus" id="dstatus" disabled="disabled" class="inputstyle" style="border-width:0px;">
				</td>
			</tr>
		</table>
	</div>
