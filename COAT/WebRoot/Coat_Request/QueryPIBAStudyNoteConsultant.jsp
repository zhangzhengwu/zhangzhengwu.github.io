    			  ...
    			  if(dataRole[3]>0){
    				  for(var i=0;i<dataRole[0].length;i++){
    				  	html+="<tr class='tr_search'><td>"+((pagenow-1)*15+(i+1))
    				  		...
    				  		+"</td><td><a href='javascript:void(0);' onclick='detail(\""+dataRole[0][i].refno+"\",\""+dataRole[0][i].staffcode+"\",\""+dataRole[0][i].staffname+"\",\""+dataRole[0][i].location+"\",\""+dataRole[0][i].status+"\");'>Detail</a>&nbsp;&nbsp;" ;
      ...
      function detail(obj,staffcode,staffname,location,status){
       ...
       function deletes(obj){
        	if(confirm("Are you sure to Delete this record?")){// 确认删除？
	    		 ...
	    		 url:"PIBAStudyNoteServlet",
	    		 type:"post",
	    		 data:{"method":"deleteCon","refno":obj,"type":"Deleted"},
	    		 success:function(date){
	    			 alert(date);
	    			 search_access(pagenow);
	    		 },error:function(){
