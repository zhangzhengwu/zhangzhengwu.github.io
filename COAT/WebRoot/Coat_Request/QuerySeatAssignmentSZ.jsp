    			  ...
    			  if(dataRole[3]>0){
    				  for(var i=0;i<dataRole[0].length;i++){
    				  	html+="<tr class='tr_search'><td>"+((pagenow-1)*15+(i+1))
    				  		...
    				  		+"</td><td><c:if test='${roleObj.search==1}'><a href='javascript:void(0);' onclick='detail(\""+dataRole[0][i].refno+"\");'>Detail</a></c:if> ";
    				  	//2015-06-04 11:49:31 去除操作时间限制
    				  		//if(dataRole[0][i].status=="Submitted" && comptime(dataRole[0][i].creatDate,GetDateStr("${SystemAcceTime}".substring(0,10).split("-"),-1)+" 12:00:00")==1 && parseFloat(("${SystemAcceTime}").substring(11,13))<12){// 大于昨天12点且当前时间小于12点 ： 可以删除
								//html+="<a href='javascript:void(0);' onclick='deletes(\""+dataRole[0][i].refno+"\");'>Delete</a>";
							//}else if(dataRole[0][i].status=="Submitted" && comptime(dataRole[0][i].creatDate,"${SystemAcceTime}".substring(0,10)+" 12:00:00")==1 && parseFloat(("${SystemAcceTime}").substring(11,13))>=12){//大于当天12点 且当前时间大于于12点 可删除
								//html+="<a href='javascript:void(0);' onclick='deletes(\""+dataRole[0][i].refno+"\");'>Delete</a>";
							//}
