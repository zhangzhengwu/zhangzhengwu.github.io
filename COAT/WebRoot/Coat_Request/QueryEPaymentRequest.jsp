			...
			success:function(data){
				...
				if(dataRole[3]>0){
					...
					for(var i=0;i<dataRole[0].length;i++){
						...
						html+="<tr id='select' title='"+i+"'><td>"+dataRole[0][i].refno+"</td><td  align='left'>"+
						dataRole[0][i].staffname+"</td><td>"+dataRole[0][i].staffcode+"</td><td align='left'>"+dataRole[0][i].location+"</td>"
						+"<td>"+dataRole[0][i].createdate+"</td><td align='center'>"+dataRole[0][i].status+"</td>" +
						" <td><a href='javascript:void(0);' onclick='Detial("+i+");' name ='1'>Detail</a>";
						html+="</td></tr>";
					}
					//html+="<tr  id='select'><td colspan='8'><span  style='color:red'>Remark:&nbsp;&nbsp;1.Daily operating time: after 12:00 noon" +
					//"&nbsp;&nbsp;2.‘Submit Date’ is greater than the record after 12 noon today, please try not to operate.</span></td></tr>";
				}
				else{
					$(".page_and_btn").hide();
		  	 	...
		  	 	success:function(date){
					...
					queryDetail(i);  /**---->>查询Detaile**************/
                 	if(downs[0][i].status=="Submitted"){
						...
						$("#completed,#void").attr("disabled",false).hide();
							...
							success:function(date){
								...
								if(dataRoles!=null){
									...
									for(var i=0;i<dataRoles.length;i++){
										htmls+="<tr class='aaa'>"+"<td height='18'>"+dataRoles[i].detailid+"</td><td>"+dataRoles[i].refno+"</td><td>"+
										dataRoles[i].productcode+"</td><td >"+dataRoles[i].productname+"</td><td>"+
										dataRoles[i].price+"</td><td>"+dataRoles[i].quantity+"</td>"+"</tr>";
										priceAll=parseFloat(priceAll)+parseFloat(dataRoles[i].price);
									}
								}else{
									htmls+="<tr class='aaa' class='product' style='color:blue;size=5px'><b>Sorry, there is no matching record.</b></td></tr>";
								}
...
<div class="e-container" id="overs" style="display: none;">
	...
	<div class="info-form" id="pay_div">
		...
		<table id="pay_table">
			<tr>
				<td class="tagnName">Payment Method</td>
				<td class="tagCont">
					<select id="method">
			...
			<tr>
				<td class="tagnName">Payment Amount</td>
				<td class="tagCont">
					<input type="hidden" id="amount"/>
					<label class="inline checkbox">
						<input type="checkbox" name="paymentAmount" value="20" id="check_money"  class="paymentAmount"/> HKD
						<span id="span_money"></span>
					</label>
					<label class="inline checkbox">
						<input id="FOC" type="checkbox" name="paymentAmount" value="0" class="paymentAmount"/> FOC
					</label>
				</td>
			</tr>
			<tr>
				<td class="tagnName">Payment Date</td>
				<td class="tagCont">
					<input type="text" id="paymentDate" onClick="return Calendar('paymentDate')" />
				</td>
			</tr>
			<tr>
				<td class="tagnName">Handled by</td>
				<td class="tagCont">
