	  	...
	  	<form id="save_form">
			<table cellpadding="0" cellspacing="0"  class="col-2-table">
				<tr>
					...
					<td class="tagCont">
						<input type="text" id="menuName" name="menuname" value="${menuInfo.menuname}" tip="Menu Name" must="true"/>
					</td>
				</tr>
				...
				<tr>
					...
					<td class="tagCont">
						<input type="text" id="menuURL" name="menuAction"  value="${menuInfo.menuAction}"  tip="Menu Url" />
					</td>
				</tr>
				...
				<tr>
					...
					<td class="tagCont">
							<select id="parentMenu" name="parentId" tip="Parent Menu" must="true">
								<option value="0" <c:if test="${menuInfo.parentId==0}"> selected="selected"</c:if> >
									顶级菜单
								</option>
								<c:forEach items="${menuList}" var="menu">
									<option value="${fn:split(menu,'~,~')[0]}" <c:if test="${menuInfo.parentId==fn:split(menu,'~,~')[0]}"> selected="selected"</c:if>   >
										&nbsp;&nbsp;&nbsp;&nbsp;-- ${fn:split(menu,'~,~')[1]}
									</option>
								</c:forEach>
							</select>
				...
				<tr>
					...
					<td class="tagCont">
						<input type="text" id="short" name="sort" maxlength="3"  value="${menuInfo.parentId==0?(menuInfo.parentshort):(menuInfo.childshort)}"  onkeydown="vailNumber(this);" onblur="vailNumber(this);" tip="Menu Sort" must="true"/>
					</td>
				</tr>
				<tr>
					<td class="tagName"><fmt:message key="public.remark" />:</td>
					<td class="tagCont">
						<textarea type="text" id="remark" name="remark" style="height: 60px; width: 170px;">${menuInfo.remark}</textarea>
					</td>
				</tr>
		  	</table>
	  	<input type="hidden" name="method" value="modifyMenu"/>
	  	<input  type="hidden" name="menuid" value="${menuInfo.menuid}"/>
	  	<input  type="hidden" name="createname" value="${menuInfo.createname}"/>
	  	<input  type="hidden" name="createDate" value="${menuInfo.createDate}"/>
	  	<input  type="hidden" name="sfyx" value="${menuInfo.sfyx}"/>
    		...
    		success:function(date){
    			...
    			if(date!=null && date!="[]"){
	    			...
	    			for(var i=0;i<dataRole.length;i++){
	    				if("${menuInfo.parentId}"==dataRole[i].split("~,~")[0])
	    					html+="<option value='"+dataRole[i].split("~,~")[0]+"' selected='selected' >&nbsp;&nbsp;&nbsp;&nbsp;--"+dataRole[i].split("~,~")[1]+"</option>";
	    				else
