...
_skin = _getArgs('skin') || 'iblue',//king 修改   iblue
...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	_init: function( config )
	{
		...
		.align( config.textAlign, config.verticalAlign ) //用于调整文本位置 Jimmy
	 ...
	 * @author Jimmy
	 */
	align: function( textAlign, verticalAlign )
	{
		var that = this, DOM = that.DOM,
			style = DOM.main[0].style;
	...
	_iframe: function( url )
	{
		...
		var load = that._fmLoad = function()
		{
			...
			// 设置iframe垂直居中(2015-01-20 Jimmy增)
			iTextAlign = config.textAlign === 'auto' ? 'center' : config.textAlign;
			iVerticalAlign = config.verticalAlign === 'auto' ? 'middle' : config.verticalAlign;
			...
			if( !that._maxState )
			{
			    ...
			    that.align( iTextAlign, iVerticalAlign ) //用于调整文本位置 Jimmy增
			    .position( config.left, config.top );
			}
...
lhgdialog.setting =
{
	...
	width: 'auto',				// 内容宽度
	height: 'auto',				// 内容高度
	align: 'auto',				// 文本水平对齐(2015-01-20 Jimmy增)
	verticalAlign: 'auto',      // 文本垂直对齐(2015-01-20 Jimmy增)
	icon: null,					// 消息图标名称
