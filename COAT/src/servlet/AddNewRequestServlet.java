...
public class AddNewRequestServlet extends HttpServlet {
	...
	Logger log=Logger.getLogger(AddNewRequestServlet.class);
	...
	//	String sfs=Util.objIsNULL(request.getParameter("sfs"))?"N":request.getParameter("sfs");
		...
		try{
			if(Integer.parseInt(rnb.getQuantity())<=0){
				...
				log.error(adminUsername+"在提交名片印刷数据删除操作失败，   没有权限   ");
				}
			}else{
...
*/					used=pd.getused(Payer)-(pd.payerNumber(Payer));//用户已印名片数量
			...
			if(Integer.parseInt(rnb.getQuantity())>0 && (rnb.getUrgent().toUpperCase().equals("Y")|| !rnb.getStaff_code().equals(rnb.getPayer()))){
				response.sendRedirect("Success.jsp");
			}else if(Integer.parseInt(rnb.getQuantity())>0 && !(rnb.getUrgent().toUpperCase().equals("Y"))){
				out.print("<script type='text/javascript'>alert('添加成功！');location.href='addNameCard.jsp';</script>");
			}
			else{
				int isRoot = new AdminDAO().getIsRoot(adminUsername);
				if(isRoot==1){
