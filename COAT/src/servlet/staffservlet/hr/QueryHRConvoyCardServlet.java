...
import servlet.staffservlet.QueryStaffRequestConvoyServlet;
import util.Page;
import dao.QueryStaffRequstDao;
import dao.impl.QueryStaffRequstDaoImpl;
import entity.RequestStaffConvoyBean;

public class QueryHRConvoyCardServlet extends HttpServlet {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	Logger logger = Logger.getLogger(QueryHRConvoyCardServlet.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			page.setAllRows(qdao.getHRConvoyRows(name, code, startDate, endDate, location, urgentCase,ET,layout, payer));//设置总行数
			page.setPageSize(10);//设置页面显示行数
			page.setCurPage(Integer.parseInt(request.getParameter("pageNow")));//获取页面当前页码
 		 	 stafflist = qdao.queryHRRequstConvoyList(name, code, startDate, endDate, location, urgentCase,ET,layout, payer,page);
