...
public class B_commissionDaoImpl_bak_2013_10_29 implements B_commissionDao{
...
" ) a on (ag.business_agent=a.handlers_address)";
			 ...
			 while(rs.next()){
				 ...
				 String recommended2=rs.getString("recommended");//第一推荐人
				 ...
				 if(rs.getString("position").indexOf("见习")>-1 || rs.getString("position").indexOf("主任")>-1){//经手人为见习或主任
						...
						if(findposition(address+" "+help).indexOf("主任")>-1||compare (rs.getString("position"),findposition(address+" "+help),positionList)){
								isError=true;//出现 主任-主任 或者见习-见习平级
								errorString.append("计算漏洞："+rs.getString("handler")+"==职位=="+rs.getString("position")+"==辅导人=="+help+"==职位=="+findposition(address+" "+help)+"==推荐人=="+recommended2+"==职位=="+findposition(address+" "+recommended2)+";\r\n");
								String[] h_a=findleader(address+" "+help);
								if(!Util.objIsNULL(h_a[0]) && help!="北京碧升"){
									help=h_a[0];
									 address=h_a[1];
								}else{//第一辅导人为北京碧升或为空时  第二辅导人自动赋值北京碧升。
									help="北京碧升";//未找到辅导人，自动赋值    北京碧升
						...
						if(!help.equals("北京碧升")){//第一辅导人不为北京碧升
								...
								dateString.append(","+findposition(address+" "+rs.getString("recommended"))+",");//第一推荐人职称
					}else{//第一辅导人为北京碧升
			...
			dateString.append(";");
			 ...
			 int adnum=0;
			String[] liwai=errorString.toString().substring(0, errorString.toString().lastIndexOf(";")).replaceAll("计算漏洞：","").split(";");
			//System.out.println(liwai[0].toString());
			//System.out.println(liwai[1].toString());
			for(int i=0;i<objd.length;i++){
				 String[] vals=new String[6];
					...
					for(int j=0;j<objx.length;j++){
						...
						for(int p=0;p<liwai.length;p++){//遍历计算漏洞----经手人为见习或主任
							...
							if(temp[0].trim().equals(objx[4].toString().trim()) && temp[2].trim().equals(objx[6].toString().trim())){
								...
								vals[0]=temp[4];//第一辅导人
								vals[1]=(Double.parseDouble(objx[7].toString())*0.015)+"";//管理津贴
								vals[2]=temp[6];//职位
								vals[3]=temp[8];//推荐人
								vals[4]=(Double.parseDouble(objx[7].toString())*0.015)+"";//推荐津贴
								if(temp.length>10)
									vals[5]=temp[10];//职位
								else
									vals[5]="北京碧升";
