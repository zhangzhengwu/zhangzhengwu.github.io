 ...
 *
 */
public class EpartConsultantReportDaoImpl implements EpartConsultantReportDao {

	Logger logger = Logger.getLogger(EpartConsultantReportDaoImpl.class);
	...
	public List<Econsreport> querybystaff(String staffcode, String startDate,
		...
		try{
			StringBuffer sal=new StringBuffer("select mapdate,recruiter,staffcode,position,staffname,convoy,time,total,cclub,penalty,noofmap,working,al_leave,sl_leave,ol_leave,borrow,emap,lateness,noshow,ontimr,e_lateness,e_noshow,e_onTime,updname,upddate from e_partconsrepot where staffcode like '%"+staffcode+"%'");
			...
			while(rs.next()){
				Econsreport es =new Econsreport(rs.getString("mapdate"), rs.getString("recruiter"), rs.getString("staffcode") ,
						...
						rs.getString("total") , rs.getString("cclub"), rs.getString("penalty"),
	...
	public ResultSet queryrsbystaff(String staffcode, String startDate,String endDate, int pageSize, int currentPage) {
		ResultSet rs= null;
		try{
			StringBuffer sal=new StringBuffer("select recruiter,staffcode,position,staffname,convoy,time,day1,day2," +
					"day3,day4,day5,day6,day7,day8,day9,day10,day11,day12,day13,day14,day15,day16,day17,day18,day19," +
					"day20,day21,day22,day23,day24,day25,day26,day27,day28,day29,day30,day31,total,cclub,penalty,''," +
					"noofmap,'',working,al_leave,sl_leave,ol_leave,borrow,emap,lateness,noshow,ontimr,e_lateness," +
					"e_noshow,e_onTime from e_partconsrepot where staffcode like '%"+staffcode+"%'");
			if(!Util.objIsNULL(startDate)){
				sal.append(" and mapdate >='"+startDate+"'");
			}if(!Util.objIsNULL(endDate)){
				sal.append(" and mapdate <='"+endDate+"'");
			}
			 logger.error("查询打卡记录       sql:===="+sal.toString());
			con=DBManager.getCon();
			ps = con.prepareStatement(sal.toString());
			rs=ps.executeQuery();
		}catch(Exception e){
			e.printStackTrace();
			logger.error("查询打开记录时出现："+e);
		}finally{
	...
	*/
	public int getCount(String staffcode, String startDate, String endDate) {
		int num=-1;
		try{
			StringBuffer sal=new StringBuffer("select count(*) as num from e_partconsrepot where staffcode like '%"+staffcode+"%'");
			if(!Util.objIsNULL(startDate)){
				sal.append(" and mapdate >='"+startDate+"'");
			}if(!Util.objIsNULL(endDate)){
				sal.append(" and mapdate <='"+endDate+"'");
	...
	public int deleteConsultantReport(String staffcode, String mapdate) {
		...
		try{
			con=DBManager.getCon();
			sql="delete from e_partconsrepot where staffcode='"+staffcode+"' and mapdate='"+mapdate+"'";
